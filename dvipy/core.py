...
# =====================================================================
# 1. BUILDING BLOCKS (Untuk user tingkat lanjut yang mau bikin loop sendiri)
# =====================================================================

def energi_eigen(dx, vpot, psi, stencil=3):
    """
    Menghitung ekspektasi energi <psi|H|psi>
    """
    vpot_arr = np.asarray(vpot, dtype=np.float64)
    psi_arr = np.asarray(psi, dtype=np.float64)
    n = len(psi_arr)
    
    # Memanggil eigene dari Fortran
...
def psi_eigen(dx, vpot, psi_old, E_guess, stencil=3):
    """
    Menyelesaikan persamaan (H - E_guess) * phi = psi_old
    """
    vpot_arr = np.asarray(vpot, dtype=np.float64)
...
# =====================================================================
# 2. MAIN SOLVER (Black-box, langsung jadi)
# =====================================================================

def solve_full(x, vpot, initial_e, psi0, stencil=3, xi=0.5, tol=1e-7, maxiter=1000):
    """
    Menjalankan iterasi solver eigenvalue 1D secara penuh di dalam Fortran.
    """
